...
FT_Bitmap_Convert           = __dll__.FT_Bitmap_Convert
...
class Bitmap(object):
    ...
    *Note*:
    ...
    def convert(self, alignment):
        target = FT_Bitmap()
        library = get_handle()
        error = FT_Bitmap_Convert(library, byref(self._FT_Bitmap), byref(target), FT_Int(alignment))
        if error: raise FT_Exception(error)
        else:
