...
if sys.platform == 'linux':
    class g15plugin(ts3plugin):
        ...
        def __init__(self):
            ...
            (err, aid) = self.g15.ttf_load(bytes(os.path.join(ts3lib.getPluginPath(), "pyTSon", "scripts", "g15", "ressources", "unifont.ttf"), 'utf-8'), 12)
        ...
        def updateDisplay(self):
            ...
            (err, myid) = ts3lib.getClientID(schid) if schid > 0 else (0, 0)
            if err == ts3defines.ERROR_ok:
                (err, chan) = ts3lib.getChannelOfClient(schid, myid) if myid > 0 else (0, 0)
                if err == ts3defines.ERROR_ok:
                    (err, channame) = ts3lib.getChannelVariableAsString(schid, chan, ts3defines.ChannelProperties.CHANNEL_NAME) if chan > 0 else (0, "TS3")
                    if err == ts3defines.ERROR_ok:
                        title = "[%s]" % channame
                        (err, away) = ts3lib.getClientSelfVariableAsInt(schid, ts3defines.ClientPropertiesRare.CLIENT_AWAY)
